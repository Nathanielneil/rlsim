...
def setup_experiment_dirs(experiment_name: str) -> dict:
    ...
    # 创建目录结构
    dirs = {
        'models': Path(f'models/dqn/{experiment_name}'),
...
def evaluate_agent(agent: DQNAgent, env: AirSimNavigationEnv, 
...
def main():
    ...
    logger.info(f"开始DQN训练实验: {experiment_name}")
    ...
    try:
        ...
        logger.info(f"网络架构: {config.get('algorithm_params', {}).get('net_arch', [512, 512, 256])}")
        logger.info(f"缓冲区大小: {agent.buffer_size}")
        logger.info(f"使用Double DQN: {agent.use_double_dqn}")
        logger.info(f"使用Dueling DQN: {agent.use_dueling}")
        logger.info(f"优先经验回放: {agent.prioritized_replay}")
    except Exception as e:
    ...
    try:
        for episode in range(1, args.episodes + 1):
            ...
            if args.visualize:
                ...
                training_data['exploration_rates'].append(agent.current_eps)
            ...
            if args.visualize and episode % (args.log_interval * 5) == 0:
                try:
                    ...
                    visualizer.plot_training_curves(
                        training_data, 
                        title=f"DQN Training Progress (Episode {episode})",
        ...
        logger.info(f"DQN训练完成! 实验: {experiment_name}")
